...
Descripción
Codexion es una implementación en C de un sistema concurrente que simula un entorno de trabajo de programadores (coders) compitiendo por recursos limitados (dongles) para realizar tareas de desarrollo. El sistema utiliza hilos POSIX (pthreads) para modelar el comportamiento concurrente de múltiples programadores trabajando en paralelo.

Cada programador necesita dos dongles para trabajar: uno propio y uno de su vecino. Los dongles tienen un tiempo de enfriamiento después de ser utilizados, lo que introduce una complejidad adicional en la gestión de recursos. El sistema implementa dos algoritmos de planificación diferentes: FIFO (First In, First Out) y EDF (Earliest Deadline First) para determinar el orden en que los programadores obtienen acceso a los dongles.

Además, el sistema incluye un hilo monitor que detecta cuando un programador se "quema" (burnout) por no poder completar sus tareas dentro del tiempo límite establecido.
...
make
Esto generará el ejecutable codexion.

Ejecución
El programa se ejecuta con el siguiente formato:

./codexion <número_de_programadores> <tiempo_para_quemarse> <tiempo_para_compilar> <tiempo_para_debuggear> <tiempo_para_refactorizar> <número_de_iteraciones> <tiempo_de_enfriamiento_dongle> <algoritmo>
Donde:

número_de_programadores: Número de programadores concurrentes

tiempo_para_quemarse: Tiempo máximo en milisegundos antes de que un programador se queme

tiempo_para_compilar: Tiempo en milisegundos que toma compilar

tiempo_para_debuggear: Tiempo en milisegundos que toma debuggear

tiempo_para_refactorizar: Tiempo en milisegundos que toma refactorizar

número_de_iteraciones: Número de ciclos completos que realizará cada programador

tiempo_de_enfriamiento_dongle: Tiempo en milisegundos que un dongle necesita para enfriarse después de ser usado

algoritmo: Puede ser "fifo" o "edf" para seleccionar el algoritmo de planificación
...
Prevención de interbloqueos y condiciones de Coffman
El sistema evita los deadlocks clásicos mediante una cuidadosa gestión de los recursos:

Orden de adquisición: Los programadores siempre intentan adquirir los dongles en un orden específico (primero su propio dongle, luego el del vecino), lo que previene el interbloqueo circular.

Timeout en espera: La función pthread_cond_timedwait se utiliza para esperar por recursos, con un timeout que permite a los hilos reaccionar a condiciones de burnout sin quedar bloqueados indefinidamente.

Liberación ordenada: Los recursos se liberan siempre en el orden inverso al de adquisición.

Prevención de inanición
El sistema implementa mecanismos para prevenir la inanición de los programadores:

En FIFO, se mantiene una cola de espera que garantiza que los programadores sean atendidos en orden de llegada.

En EDF, se prioriza a los programadores con deadlines más cercanos, pero el sistema está diseñado para que todos eventualmente obtengan acceso a los recursos necesarios.

Gestión del cooldown
El tiempo de enfriamiento de los dongles se gestiona mediante una marca de tiempo available_at_ms que indica cuándo estará disponible nuevamente un dongle. Los programadores verifican esta marca de tiempo antes de intentar adquirir un dongle.

Detección precisa del agotamiento
El hilo monitor verifica continuamente si algún programador se ha quemado comparando el tiempo actual con el tiempo de inicio de la última compilación más el tiempo límite para quemarse. Esta detección es precisa gracias al uso de gettimeofday para obtener tiempos en milisegundos.

Serialización del log
Toda la salida de registro se serializa para evitar que los mensajes de diferentes hilos se mezclen. Esto se logra mediante el uso de mutexes alrededor de las operaciones de impresión.

Thread synchronization mechanisms
Primitivas de hilos utilizadas
pthread_mutex_t: Se utiliza para proteger el acceso a recursos compartidos como:

Los dongles (protegiendo sus estados taken y available_at_ms)

La cola de espera de cada dongle

El estado del monitor (variable someone_burned)

pthread_cond_t: Se utiliza para la comunicación entre hilos:

Cada dongle tiene una variable de condición para notificar a los programadores en espera cuando se libera

Cada solicitud en la cola de espera tiene su propia variable de condición para notificaciones individuales

El monitor utiliza una variable de condición para coordinar el final de la simulación

Coordinación de acceso a recursos compartidos
Dongles
Cada dongle está protegido por su propio mutex, lo que permite acceso concurrente a diferentes dongles mientras se serializa el acceso a cada uno en particular. Las operaciones de adquisición y liberación siguen este patrón:

Bloquear el mutex del dongle

Verificar condiciones (disponibilidad, tiempo de enfriamiento)

Realizar la operación (tomar o liberar)

Notificar a hilos en espera si es necesario

Liberar el mutex

Logging
El logging se serializa mediante el uso de mutexes alrededor de las operaciones de impresión, asegurando que los mensajes completos de cada hilo se impriman sin interferencias.

Estado del monitor
El estado compartido del monitor (variable someone_burned) está protegido por un mutex que garantiza la visibilidad de los cambios a todos los hilos.

Evitando condiciones de carrera
Las condiciones de carrera se evitan mediante:

Uso consistente de mutexes para proteger todas las variables compartidas

Operaciones atómicas en estructuras de datos compartidas (como la inserción y eliminación en colas de espera)

Uso de variables de condición para coordinar el acceso bloqueado a recursos

Comunicación thread-safe
La comunicación entre programadores y monitor se realiza de manera thread-safe mediante:

Variables de estado compartidas protegidas por mutexes

Señales a través de variables de condición cuando cambian condiciones importantes

Verificaciones periódicas de condiciones de terminación
...
Uso de IA
La inteligencia artificial se utilizó en las siguientes tareas durante el desarrollo del proyecto:

Ayuda en la comprensión de conceptos de concurrencia: Para entender mejor cómo funcionan los mutexes, variables de condición y cómo prevenir deadlocks.

Generación de casos de prueba: Para crear escenarios específicos que ayudaran a validar el comportamiento correcto de FIFO y EDF.

Depuración de errores de sincronización: Para identificar posibles condiciones de carrera y sugerir patrones de solución.

Optimización de estructuras de datos: Para mejorar la eficiencia de las colas de espera en los algoritmos de planificación.

Revisión de código: Para detectar posibles problemas de memoria o liberación incorrecta de recursos.

El uso de IA fue principalmente como herramienta de aprendizaje y verificación, no para generar el código base del proyecto. Toda la implementación fue realizada manualmente como parte del currículo de 42.Codexion
Este proyecto ha sido creado como parte del currículo de 42 por aitorres.
...
El uso de IA fue principalmente como herramienta de aprendizaje y verificación, no para generar el código base del proyecto. Toda la implementación fue realizada manualmente como parte del currículo de 42.
